...
public class MoveConstructor {
    ...
    public List<IdentificatedButton> nextButtonsLine(byte[] chessboard,
                                                     ...
                                                     String[] lettersNames, String[] digitsNames, GameHandler gameHandler) {
        switch (currentStatus) {
            ...
            case MoveConstructor.STATUS.START:
                return possibleMoves(chessboard, whiteToMove,
                        lettersNames, digitsNames, gameHandler);
            default:
                return new ArrayList<IdentificatedButton>();
        }
    }
    ...
    private List<IdentificatedButton> figuresLeft(byte[] chessboard,
        ...
        List<IdentificatedButton> buttons = new ArrayList<IdentificatedButton>();
    ...
    private List<IdentificatedButton> possibleMoves(
        ...
        int squaresInARow = (int) Math.sqrt(squaresCount);
        List<IdentificatedButton> buttons = new ArrayList<IdentificatedButton>();
        List<Integer> possibleMoves = gameHandler.everyPossibleRightMove(figure, startPosition, chessboard, whiteToMove);
        for (int move : possibleMoves) {
            buttons.add(new IdentificatedButton(
                    lettersNames[move % squaresInARow] +
                            digitsNames[(squaresInARow - move / squaresInARow) - 1],
                    String.valueOf((char) move)));
        }
